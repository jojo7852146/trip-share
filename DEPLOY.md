...
**文字/行程/费用等所有数据都存在 Postgres，永久保存。但"本地上传"的图片
存在服务器的临时磁盘上**，遇到以下情况会丢失：

- 你 `git push` 新代码 → Render 自动重新部署 → 磁盘重置 → 图片没了
- Render 实例迁移 / 重启（偶尔发生）

**建议**：
- 重要的照片尽量用"图片 URL"方式（链接指向你自己的图床/相册），不依赖本服务存储
- 本地上传适合临时配图；要长期保存，等部署后我帮你接一个免费图床（如
  Cloudinary / 腾讯云 COS / 七牛），改一行上传代码即可
